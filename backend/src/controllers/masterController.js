...
exports.getSekolah = async (req, res) => { //mendapatkan semua data sekolah
  try { // mencoba menjalankan kode berikut
    const [rows] = await pool.query(
      `SELECT id_sekolah, uuid, nama_sekolah, alamat, telepon, created_at
       FROM sekolah
       ORDER BY nama_sekolah`
    ); //menjalankan query untuk mendapatkan semua data sekolah
    res.json(rows); //mengirim respons dengan data sekolah dalam format JSON
  } catch (err) { //menangani kesalahan
    console.error("getSekolah error:", err);
...
exports.createSekolah = async (req, res) => { //membuat data sekolah baru
  ...
  try { 
    ...
    const [rows] = await pool.query(
      `SELECT id_sekolah, uuid FROM sekolah WHERE id_sekolah = ? LIMIT 1`,
      [result.insertId]
    );
    res.status(201).json({ 
      ...
      uuid: rows[0].uuid,
     }); //mengirim respons 201 dengan id sekolah yang baru dibuat
  } catch (err) {
    console.error("createSekolah error:", err);

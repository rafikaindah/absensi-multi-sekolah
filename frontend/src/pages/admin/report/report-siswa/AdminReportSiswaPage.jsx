...
import { useAdminReportSiswaController } from "./adminReportSiswa.controller";
import "../../../guru/Report.css";

export default function AdminReportSiswaPage() {
  const { toggleSidebar } = useOutletContext();
  const c = useAdminReportSiswaController();
  ...
  return (
    <div className="rpage">
      ...
      <div className="rhead">
        ...
        <div><h1>Report Siswa</h1></div>
      </div>

      {/* pesan error */}
      {c.error && <div className="ralert">{c.error}</div>}

      {/* card (start, end, guru, sekolah, kelas + tombol terapkan filter) */}
      <div className="rcard">
        <div className="rfilters">
          ...
          {/* filter guru (admin wajib pilih guru) */}
          <div className="rfield">
            <label>Nama Guru</label>
            <select 
              value={c.filter.id_pengguna} 
              onChange={(e) => c.onChange("id_pengguna", e.target.value)}
            >
              <option value="">Pilih Guru</option>
              {c.guruList.map((g) => (
                <option key={g.id_pengguna} value={g.id_pengguna}>
                  {g.nama}
                </option>
              ))}
            </select>
